%
% gic rank fast version by Toshinari Morimoto
%
...
function [r_gic,gic,likelihood, b] = gic_rank_fast(x,upperbound)
...
E = repmat(reshape(D,[p,p,1,T]),[1,1,p,1]); % [p,p,p,T]: Copy D p times (the 3rd axis can be seen as r=0,...,p-1)
...
range = pagemtimes(reshape(triu(ones(p,p)-eye(p)),[p,1,p]),reshape(tril(ones(p,p)),[1,p,p]));
range = repmat(reshape(range,[p,p,p,1]),[1,1,1,T]);

% compute inverse of li-lj on over the range of summation. 
% want to define 0^-1 as 0
F = 1/(range .* E + epsilon);
idx = (F <= inf);
F = F.*idx;
...
m = repmat(reshape(ll,[p,1,T]),[1,p,1]) .* repmat(reshape(inv_sigma_r2,[1,p,T]),[p,1,1]) - 1;

b_part1 = reshape(sum(m .* H,1),[p,T]); % [p,T]
